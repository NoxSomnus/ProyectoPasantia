...
{
    internal class ProcessInscriptionsCSVFileQueryHandler : IRequestHandler<ProcessInscriptionsCSVFileQuery, AddInscriptionsResponse>
    {
        private readonly ILogger<ProcessInscriptionsCSVFileQueryHandler> _logger;
        ...
        /// <summary>
        /// Constructor de la clase ProcessInscriptionsCSVFileQueryHandler.
        /// </summary>
        ...
        public ProcessInscriptionsCSVFileQueryHandler(ILogger<ProcessInscriptionsCSVFileQueryHandler> logger,
        ...
        public Task<AddInscriptionsResponse> Handle(ProcessInscriptionsCSVFileQuery request, CancellationToken cancellationToken)
        {
            try
            {
                if (request == null)
                {
                    _logger.LogWarning("ProcessInscriptionsCSVFileQueryHandler.Handle: Request null.");
            ...
            {
                _logger.LogWarning("ProcessInscriptionsCSVFileQueryHandler.Handle: ArgumentNullException");
                throw;
            }
        ...
        public async Task<AddInscriptionsResponse> HandleAsync(ProcessInscriptionsCSVFileQuery request)
        {

            try
            {
                _logger.LogInformation("ProcessInscriptionsCSVFileQueryHandler.HandleAsync");
                ...
                List<AddInscriptionRequest> inscriptions = new List<AddInscriptionRequest>(); //CAMBIAR ESTO
                ...
                using (var fileStream = new MemoryStream(fileBytes))
                {
                    ...
                    using (StreamReader reader = new StreamReader(fileStream))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            try
                            {
                                ...
                                AddInscriptionRequest inscription = new AddInscriptionRequest
                                {
                                    PeriodName = columns[0],
                                    PeriodYear = int.Parse(columns[1]),
                                    CourseName = columns[2],
                                    Regularidad = (RegularidadEnum)Enum.Parse(typeof(RegularidadEnum), columns[3]),
                                    ModulName = columns[4],
                                    Turno = (TurnosEnum)Enum.Parse(typeof(TurnosEnum), columns[5]),
                    ...
                    var filerequest = new AddInscriptionByCSVRequest
                    {
                        Inscriptions = inscriptions
                    };

                    _rabbit.SendProductMessage(filerequest);
                    var response = await _mediator.Send(new AddInscriptionsByCSVCommand(filerequest));
            ...
            {
                _logger.LogError(ex, "Error ProcessInscriptionsCSVFileQueryHandler.HandleAsync. {Los datos ingresados no son validos}", ex.Message);
                throw;
            }
        }
    }
